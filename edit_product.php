<?php
// เปิดการแสดง Error ไว้ชั่วคราวเพื่อให้ตรวจสอบได้ง่าย
...
// ดึงข้อมูลสินค้าเดิม (ค้นหาด้วย id ของสินค้าอย่างเดียว เนื่องจากตารางไม่มี user_id)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $image_path = $product['image']; // ใช้รูปเดิมเป็นค่าเริ่มต้น
    ...
    if (empty($name) || empty($price) || empty($stock) || empty($category_id)) {
        ...
        // อัปเดตข้อมูลสินค้าโดยตัด user_id ออก
        $stmt = $pdo->prepare("UPDATE products SET name = ?, price = ?, stock = ?, category_id = ?, description = ?, image = ? WHERE id = ?");
        if ($stmt->execute([$name, $price, $stock, $category_id, $description, $image_path, $product_id])) {
            header("Location: seller_center.php");
            exit();
        } else {
...
<head>
    ...
    <title>แก้ไขข้อมูลสินค้า - Hugmart</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Prompt', sans-serif; }
        ...
        .current-image { display: flex; align-items: center; gap: 15px; margin-bottom: 10px; }
        .current-image img { width: 60px; height: 60px; object-fit: cover; border-radius: 8px; border: 1px solid var(--border); }
    ...
    <div class="container">
        <div class="card">
            <div class="card-header">
                <h2><i class="fa-solid fa-pen-to-square"></i> แก้ไขข้อมูลสินค้า</h2>
            ...
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="stock" class="form-control" min="0" required value="<?= htmlspecialchars($product['stock']) ?>">
                </div>
                ...
                <div class="form-group">
                    <label class="form-label">รูปภาพสินค้า (เปลี่ยนใหม่หรือปล่อยว่างไว้เพื่อใช้รูปเดิม)</label>
                    <?php if (!empty($product['image'])): ?>
                        <div class="current-image">
                            <img src="<?= htmlspecialchars($product['image']) ?>" alt="Product Image">
                            <span style="font-size: 0.85rem; color: var(--text-muted);">รูปภาพปัจจุบัน</span>
                        </div>
                    <?php endif; ?>
